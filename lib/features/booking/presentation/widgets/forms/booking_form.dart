  ...
  void _navigateToChooseCarPage() {
    if (_formKey.currentState!.validate()) {
      ...
      Rent initialRent = Rent(
        ...
        driverId: _isNeedDriver ? _driverDropdownController.dropDownValue?.value : null,
        driverName: _isNeedDriver ? _driverDropdownController.dropDownValue?.name : null,
      );
            ...
            child: Column(
              children: [
                ...
                FormDropdownField(
                  ...
                  isRequired: true,
                  items: needList.map((e) => DropDownValueModel(name: e.name, value: e.name)).toList(),
                ),
                SizedBox(height: 6.h),
                FormTextareaField(
                  ...
                  BlocBuilder<DriverBloc, DriverState>(builder: (context, state) {
                    if (state is DriverGetDriversSuccess) {
                      return FormDropdownField(
                        ...
                        isRequired: true,
                        items: state.drivers!.map((e) => DropDownValueModel(name: e.name, value: e.id)).toList(),
                      );
                    }
